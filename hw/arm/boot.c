...
static void arm_load_kernel_notify(Notifier *notifier, void *data)
{
    ...
    printf("arm_load_elf: kernel_size: %d\n", kernel_size);
    ...
    if (arm_feature(&cpu->env, ARM_FEATURE_AARCH64) && kernel_size < 0) {
        ...
        printf("load_aarch64_image: kernel_size: %d\n", kernel_size);
        is_linux = 1;
    ...
    if (is_linux) {
        ...
        if (info->initrd_filename) {
            initrd_size = load_ramdisk_as(info->initrd_filename,
                                          ...
                                          as);
            printf("load_ramdisk_as: initrd_size: %d\n", initrd_size);
            if (initrd_size < 0) {
                initrd_size = load_image_targphys_as(info->initrd_filename,
                                                     ...
                                                     info->initrd_start,
                                                     as);
                printf("load_image_targphys_as: initrd_size: %d\n", initrd_size);
            }
            if (initrd_size < 0) {
